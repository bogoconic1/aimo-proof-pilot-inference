"""Load and strictly validate a Proof Pilot OpenRouter runtime YAML."""
...
ROOT_KEYS = {"schema_version", "provider", "search"}
PROVIDER_KEYS = {"base_url", "model", "api_key_env", "reasoning_effort"}
SEARCH_KEYS = {
...
def load_config(path: Path) -> dict[str, Any]:
    ...
    if config["schema_version"] != 13:
        raise ValueError("schema_version must be 13")
    for section, keys in (
        ("provider", PROVIDER_KEYS), ("search", SEARCH_KEYS),
    ):
    ...
    for key in ("base_url", "model", "api_key_env", "reasoning_effort"):
        if not isinstance(provider[key], str) or not provider[key]:
            raise ValueError(f"provider.{key} must be a nonempty string")
    if provider["base_url"] != "https://openrouter.ai/api/v1":
        raise ValueError(
            "provider.base_url must be https://openrouter.ai/api/v1"
        )
    if provider["model"] != "deepseek/deepseek-v4-flash":
        raise ValueError(
            "provider.model must be deepseek/deepseek-v4-flash"
        )
    if provider["api_key_env"] != "OPENROUTER_API_KEY":
        raise ValueError("provider.api_key_env must be OPENROUTER_API_KEY")
